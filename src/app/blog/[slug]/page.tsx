...
// Pre-render static paths for blogs during generation
...
// SEO metadata tags for dynamic articles
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const resolvedParams = await params;
  const { slug } = resolvedParams;
  const blog = BLOGS.find((b) => b.slug === slug);
  
  if (!blog) return { title: "Blog Article | Himalayan Warrior" };

  return {
    title: `${blog.title} | Himalayan Warrior`,
    description: blog.excerpt,
    keywords: [...blog.tags, "Himalayan guide", "hiking guide", "trek tips"],
  };
}
          ...
          <div className="space-y-4 mb-8">
            ...
            <div className="flex items-center gap-2.5 pt-2">
              ...
              <div>
                ...
                <span className="text-[10px] text-muted-foreground">Expedition Lead</span>
              </div>
            </div>
          </div>
