...
export function BookingForm({ trek, onSuccess }: BookingFormProps) {
  ...
  const saveBookingLocally = (booking: any) => {
    try {
      const existing = localStorage.getItem("himalayanwarrior_bookings") || "[]";
      const bookingsList = JSON.parse(existing);
      bookingsList.push({
        ...booking,
        id: "HW-" + Math.floor(100000 + Math.random() * 900000),
        createdAt: new Date().toISOString(),
      });
      localStorage.setItem("himalayanwarrior_bookings", JSON.stringify(bookingsList));
    } catch (e) {
          ...
          >
            <div className="text-center sm:text-left mb-2">
              <p className="text-xs text-muted-foreground leading-relaxed">
                As per Uttarakhand state forest department guidelines, uploading your identity proof (Aadhaar/Passport) and a health self-declaration is mandatory to secure trek permit approvals at checkpoints.
              </p>
            </div>
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-muted-foreground max-w-sm mx-auto leading-relaxed">
                Thank you for choosing Himalayan Warrior, <span className="font-semibold text-foreground">{fullname}</span>. Your slot for <span className="font-semibold text-foreground">{trek.title}</span> on <span className="font-semibold text-foreground">{trekDate}</span> is registered!
              </p>
            </div>
            ...
            <div className="border border-border rounded-xl p-4 bg-secondary/40 text-left text-xs max-w-sm mx-auto divide-y divide-border/60">
              ...
              <div className="flex justify-between py-2">
                <span className="text-muted-foreground font-semibold">Amount Due at Base</span>
                <span className="font-bold text-primary text-sm">₹{totalPrice}</span>
              </div>
            </div>

            <p className="text-[10px] text-muted-foreground">
              A copy of your booking registration voucher and assembly instructions has been dispatched to {email}. For urgent support, call +91-7249936622.
            </p>
          </motion.div>
        )}
      </AnimatePresence>
